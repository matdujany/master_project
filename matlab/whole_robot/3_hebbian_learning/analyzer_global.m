...
desired_movement_speed_channel=2;
n_limb=size(limb,1);
...
plot_limb_to_lc_effect(z_effect_limb_to_lc,parms,['Direct map for movement in direction ' direction_list{desired_movement_speed_channel}]);

%% inverse map for tegotae
z_effect_lc_to_limb = z_effect_limb_to_lc'/100;
z_effect_lc_to_limb = z_effect_lc_to_limb/max(max(abs(z_effect_lc_to_limb))) ;

plot_lc_to_limb_inv_map(z_effect_lc_to_limb,parms,['Inverse map for movement in direction ' direction_list{desired_movement_speed_channel}])
%% scaling sigma
frequency = 0.5;
total_load = 16;
...
%%
motors_classes_for_robotis = motors_classes - 1
changeDirs_for_robotis = (dir_oscillations == -1)