...
for i=1:n_limb
    for i_lc = 1:parms.n_lc
        ...
        weights_limb_summedz(i,i_lc) = sum(abs(weights_lc(index_list,3*i_lc)))/4;
    end
end
...
for i_limb=1:4
    for i_lc=1:4
        text(i_lc-0.5,5-i_limb-0.5,num2str(weights_limb_summedz(i_limb,i_lc),'%.2f'),'FontSize',fontSize,'HorizontalAlignment','center');
    end
end
h2.Color = 'w';
