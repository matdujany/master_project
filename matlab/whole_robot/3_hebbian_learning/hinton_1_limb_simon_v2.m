...
% hinton_pos(weights_pos_read{n_iter},parms,1)
...
weights_gyro = weights_read{n_iter}(end-2:end,idx_motor_wm);
weights_speed = weights_speed_all{n_iter}(1:3,idx_motor_wm);

renorm_pos = max(max(abs(weights_pos_read{n_iter})));
renorm_lc = max(max(abs(weights_read{n_iter}(1:3*(parms.nr_arduino),:))));
renorm_gyro = max(max(abs(weights_read{n_iter}(end-2:end,:))));
renorm_speed = max(max(abs(weights_speed_all{n_iter}(1:3,:))));

weights_motor_rescaled = weights_motor/renorm_pos;
weights_lc_rescaled = weights_lc/renorm_lc;
weights_gyro_rescaled = weights_gyro/renorm_gyro;
weights_speed_rescaled = weights_speed/renorm_speed;

%%
weights = [weights_lc; weights_speed; weights_gyro];
weights_rescaled = 100*[weights_lc_rescaled; weights_speed_rescaled; weights_gyro_rescaled];

%%
addpath('../../export_fig');
%%
h1 = hinton_limb(weights,parms,1);
...
set(h2,'Position',[10 10 800 1000]);
% export_fig 'figures_simon_2/hinton_limb_rescaled.pdf'
...
x_shift = 0.15;
row_names_list = {'Loadcell X','Loadcell Y','Loadcell Z','Speed X','Speed Y','Speed Z','Gyro. Roll','Gyro. Pitch','Gyro. Yaw'};
