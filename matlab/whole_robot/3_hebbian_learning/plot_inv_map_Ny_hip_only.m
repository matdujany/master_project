function h=plot_inv_map_Ny_hip_only(weights_inv_map)
...
text((x_min+x_max)/2,-0.5,'$$ \dot{\phi_i} = \omega - \sigma_p\sum_{all\ j} v_{i,j}^{hip} F_j cos(\phi_i),   (\sigma_p>0) $$','FontSize',fontSize+5,'HorizontalAlignment','center','Interpreter','latex');
text(-0.5,y_max+0.2,'$$ v_{i,j}^{hip} $$','FontSize',fontSize+5,'HorizontalAlignment','center','Interpreter','latex');
