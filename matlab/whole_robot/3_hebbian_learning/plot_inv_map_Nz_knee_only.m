function h=plot_inv_map_Nz_knee_only(weights_inv_map)
...
text((x_min+x_max)/2,-0.5,'$$ \dot{\phi_i} = \omega + \sigma\sum_{all\ j} u_{i,j}^{knee} N_j sin(\phi_i),   (\sigma>0) $$','FontSize',fontSize+5,'HorizontalAlignment','center','Interpreter','latex');
text(-0.5,y_max+0.2,'$$ u_{i,j}^{knee} $$','FontSize',fontSize+5,'HorizontalAlignment','center','Interpreter','latex');
