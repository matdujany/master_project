...
actual_duration = recorded_data(end-3);
amplitude_oscillations = recorded_data(end-2);
frequency_start = recorded_data(end-1);
frequency_end = recorded_data(end);

motor_position = recorded_data(1:2:2*nb_samples);
command_position = recorded_data(2:2:2*nb_samples+1);
...
plot(time_ms,command_position);

xlabel('Time [ms]');
ylabel('Motor Position [Pos]');
legend('Actual Position','Command Position');
